...
function save(){ localStorage.setItem(STORAGE, JSON.stringify(data)); syncToSheet(); }
...
function openModal(html){ const m=document.getElementById("modal"); m.innerHTML=html; document.getElementById("modalBackdrop").classList.remove("hidden"); // trigger animation
...
function timelineClick(i){
  ...
  // small delay then open modal with details
    setTimeout(()=>{
...
function editEvent(id){
 ...
 let severity=[[
  "只係有啲唔gur",-2,"🙂"],
 ["有啲嬲",-5,"😐"],["幾嬲吓",-10,"😤"],["真係嬲",-15,"😡"],["巨嬲",-20,"🤬"]];
 let good=[[
  "幾好",2,"🙂"],["好貼心",5,"😊"],["非常貼心",10,"🥰"],["今日最佳",15,"👑"],["神級男友",20,"💎"]];
 const levels=bad?severity:good;
 const cats=bad?data.settings.categories:["體貼","送禮","陪食飯","陪伴","關心","幫手","特別安排","其他"];
...
function getMonthlyReviewLabel(){
 const info=getMonthlyReviewWindow();
 return info.monthLabel;
}
...
function achievementList(){
 ...
 // count severe bad events (severity "真係嬲" or "巨嬲") across all time
 ...
 return [
  ...
  // 黑歷史 now counts only severe 嬲事 (真係嬲 + 巨嬲)
  {icon:"💀",name:"黑歷史",desc:severeAllTimeCount>=50?"已解鎖：黑歷史 50/50":`真係嬲+巨嬲 ${severeAllTimeCount}/50`},
  // additional achievement for extreme black history
...
function settingsPage(){
 ...
 <div class="card"><h2 style="margin-top:0">⚖️ 分數設定</h2>
  ...
  <div class="card" style="margin-top:18px"><h2 style="margin-top:0">💾 資料管理</h2><div class="actions"><button class="btn" onclick="exportData()">匯出 JSON</button><label class="btn">匯入 JSON<input type="file" accept=".json" onchange="importData(event)" style="display:none"></label><button class="btn filled danger" onclick="resetData()">清除所有資料</button></div><div class="footer-note">資料會儲存喺瀏覽器 localStorage。</div></div>`;
}
function saveSettings(){data.boyfriend.name=document.getElementById("sname").value||"男朋友";data.boyfriend.nickname=document.getElementById("snick").value||"BF";data.boyfriend.startDate=document.getElementById("sdate").value||data.boyfriend.startDate;data.threshold=Math.max(1,Math.min(99,Number(document.getElementById("sth").value)||45));save();toast("設定已儲存。");render("dashboard");}
